...
'''for i in range(int(len(b6z['score'])/32)):
    avg = 0
    for j in b6z['score'][i*32:((i+1)*32)]:
        avg+= abs(j)
    avg = avg/32
    #print (b6z['score'][i*32:((i+1)*32)])
    print (i, avg)
'''
...
def average(x):
    score=[]
    cs = 32
    for i in range(int(len(x['score'])/cs)):
        avg = 0
        for j in x['score'][i*cs:((i+1)*cs)]:
            avg+= abs(j)
        avg = avg/cs
        score.append(avg)
    #print(score)
...
ar= [bz, dz, qz, tz, uz, c4z, cz, c2z, c3z, b4z, b5z, b6z]
t_arr=[]
for ai in ar:
    x = average(ai)
    t_arr.append(x)
